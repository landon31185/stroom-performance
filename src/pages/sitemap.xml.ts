...
export const GET: APIRoute = async () => {
  const [products, guides, builds, categories] = await Promise.all([
    getCatalogProducts(), getGuides(), getBuilds(), getCollection('categories'),
  ]);
  const urls = new Set(['/', '/about', '/shop', '/guides', '/builds']);
  products.filter((product) => product.status !== 'draft').forEach((product) => urls.add(`/products/${product.slug}`));
  guides.forEach((guide) => urls.add(`/guides/${guide.slug}`));
  builds.forEach((build) => urls.add(`/builds/${build.slug}`));
